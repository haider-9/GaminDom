  ...
  const loadFavorites = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const formattedFavorites = userFavorites.map(
        ...
        platforms:
          game.platforms
            ?.map((p) => {
              ...
              return (
                (platform.platform as Record<string, unknown>)?.name ||
                platform.name
              );
            })
            .filter(Boolean) || [],
  ...
  const removeFromFavorites = async (gameId: number): Promise<void> => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        const userFavorites = data.user.favourites || [];
        const gameData = userFavorites.find(
          (fav: Record<string, unknown>) => fav.rawgId === gameId,
        );

        if (gameData) {
          const deleteResponse = await fetch(
            `/api/users/${userId}/favorites?gameId=${gameData._id}`,
            {
              method: "DELETE",
            },
          );
