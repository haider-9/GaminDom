    ...
    return (
      <div className="w-full max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-wrap gap-3">
          {[...Array(12)].map((_, i) => (
            <div key={i} className="h-10 bg-black/50 rounded-full w-24 animate-pulse"></div>
          ))}
        </div>
      </div>
    );
    ...
    <div className="w-full max-w-6xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-3xl font-bold text-white">Popular Tags</h2>
        </div>
        <button
      ...
      <div className="flex flex-wrap gap-3">
        {tags.map((tag, index) => (
          <button
            ...
            className={`group relative px-6 py-3 rounded-full text-white font-semibold transition-all duration-300 hover:scale-105 bg-gradient-to-r ${getTagColor(index)} overflow-hidden`}
          >
